...
with open("all_Couvillion_Ranges_for_RNAi_FROM2006.gtf",'r') as openhandle:  # open old gtf file
    ...
    for i in reader:
    	gtf_lines.append(i)  # create a data structure that is [['item0','item0'],['item0','item0']]
...
for new_anot_key in updated_annotations:  # for every annotation to update
    for line in gtf_lines:
        if new_anot_key == line[8].split('"')[1]:  # if annotation name in gtf = annotation name in hash table
            line[0]=updated_annotations[new_anot_key][0]  # write the scaffold
            line[3]=str(updated_annotations[new_anot_key][1])  # write the start position
            line[4]=str(updated_annotations[new_anot_key][2])  # write the end position
